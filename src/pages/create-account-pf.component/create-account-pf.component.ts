...
export class CreateAccountPfComponent implements OnInit {
  ...
  // TODO: campos não carregam junto com cep
  consultarCep(): void {
    const cepControl = this.cep;

    // Se o controle não existir ou não tiver um valor, não fazemos nada.
    if (!cepControl || !cepControl.value) {
      return;
    }

    // Verificamos os erros de formato síncronos. Se existirem, paramos aqui.
    if (cepControl.hasError('required') || cepControl.hasError('pattern')) {
      return;
    }

    // Se o formato do CEP estiver correto, prosseguimos para consultar a API e preencher os campos.
    // O validador assíncrono que já está no campo cuidará de exibir a mensagem de erro se o CEP não for encontrado.
    this.viaCepService.consultarCep(cepControl.value.replace(/\D/g, '')).subscribe({
      next: (dados) => {
        if (!dados.erro) {
  ...
  onSubmit(): void {
    ...
    this.clienteService.cadastrarClienteFisico(requestData).subscribe({
      ...
      error: (err) => {
        ...
        this.snackBar.open('Erro ao realizar o cadastro. Tente novamente.', 'Fechar', { duration: 5000 });
      }
    });
  }
}
